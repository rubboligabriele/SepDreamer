    ...
    def eval(self, episodes, epoch):
        ...
        with torch.no_grad():
            for stay_id, data in tqdm(episodes.items()):
                ...
                features = self._flatten(data["features"])
                if self._config.fm["use_fm"]:
                    delta = self._flatten(data["delta"])
                else:
                    delta = None

                embed = self._wm.encoder(features, delta)
                embed = self._unflatten(embed, B, T)
                ...
                full_states, _ = self._wm.dynamics.observe(embed, phys_action, is_first, debug=debug_now)                   # [B, T-1]
                ...
                value_estimates.append(float(tools.to_np(value_pred.squeeze())))
                ...
                real_phys_return = tools.to_np(data["reward"][:, 5:].sum(dim=1).squeeze())
                ...
                imag_rewards += float(tools.to_np(imag_reward.sum()))
                ...
                if traj_ope is not None:
                    traj_ope = self._augment_traj_debug(traj_ope, clin_action_list, stay_id)
    ...
    def eval_wm(self, episodes, epoch):
        ...
        with torch.no_grad():
            for stay_id, data in tqdm(episodes.items()):
                ...
                features = self._flatten(data["features"])
                delta = self._flatten(data["delta"]) if self._config.fm["use_fm"] else None
                ...
                phys_episode_return = tools.to_np(reward_prior.sum(dim=1).squeeze())
                phys_episode_returns.append(phys_episode_return)
                mortalities.append(tools.to_np(data["mortality"][:, 0].squeeze()))
    ...
    def _eval(self, episodes):
        ...
        with torch.no_grad():
            for stay_id, data in tqdm(episodes.items()):
                ...
                reward_head_prior = self._wm.heads["reward"](feat_prior)
                reward_prior = reward_head_prior.mode()
                phys_episode_return = tools.to_np(data["reward"][:, 5:].sum(dim=1).squeeze())
